...
class NetworkPlannerRunner(object):
    ...
    i.e. This is a wrapper for running the NetworkPlanner process with it's network 
    algorithm replaced by networker's algorithm
    ...
    def run(self):
        ...
        metric_config = json.load(open(self.config['metric_model_parameters_file']))
    ...
    def _store_networks(self, msf, existing=None):
        ...
        for subgraph in nx.connected_component_subgraphs(msf):
            ...
            for u, v, data in subgraph.edges(data=True):
                ...
                for i, fake in enumerate([n for n in edge if msf.node[n]['budget'] == np.inf], 1):
                    dataset_node = self.store.addNode(msf.coords[fake], is_fake=True)
    ...
    def _save_output(self, metric_value_by_option_by_section, metric_config, metric_model):
...
def dataset_store_to_nx_graph(dataset_store):
    ...
    G = GeoGraph(input_proj, coords_dict)
